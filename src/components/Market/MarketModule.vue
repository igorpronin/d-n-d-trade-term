<template>
  <div class="resizable-component" :style="style" ref="cmp">
    <slot></slot>
...
export default {
  ...
  mounted() {
    ...
    this.$refs.cmp.addEventListener('mousedown', this.elHandleDown, true);
    this.$refs.cmp.addEventListener('touchstart', this.elHandleDown, true);
  ...
  beforeDestroy() {
    ...
    this.$refs.cmp.removeEventListener('mousedown', this.elHandleDown, true);
    this.$refs.cmp.removeEventListener('touchstart', this.elHandleDown, true);
  ...
  methods: {
    setMaximize(value) {
      ...
      if (value) {
        this.prevState = {
          w: this.w, h: this.h, l: this.l, t: this.t,
        };
        this.t = this.l = 0;
    ...
    handleMove(event) {
      if (this.resizeState !== 0) {
        ...
        if (this.resizeState & ELEMENT_MASK['resizable-r'].bit) {
          if (!this.dragState && this.w + diffX < this.minW) this.offsetX = (diffX - (diffX = this.minW - this.w));
          else if (!this.dragState && this.maxW && this.w + diffX > this.maxW && (!this.fitParent || this.w + this.l < this.parent.width)) this.offsetX = (diffX - (diffX = this.maxW - this.w));
          else if (this.fitParent && this.l + this.w + diffX > this.parent.width) this.offsetX = (diffX - (diffX = this.parent.width - this.l - this.w));
          this.calcMap & CALC_MASK.w && (this.w += this.dragState ? 0 : diffX);
        }
        if (this.resizeState & ELEMENT_MASK['resizable-b'].bit) {
          if (!this.dragState && this.h + diffY < this.minH) this.offsetY = (diffY - (diffY = this.minH - this.h));
          else if (!this.dragState && this.maxH && this.h + diffY > this.maxH && (!this.fitParent || this.h + this.t < this.parent.height)) this.offsetY = (diffY - (diffY = this.maxH - this.h));
          else if (this.fitParent && this.t + this.h + diffY > this.parent.height) this.offsetY = (diffY - (diffY = this.parent.height - this.t - this.h));
          this.calcMap & CALC_MASK.h && (this.h += this.dragState ? 0 : diffY);
        }
        if (this.resizeState & ELEMENT_MASK['resizable-l'].bit) {
          if (!this.dragState && this.w - diffX < this.minW) this.offsetX = (diffX - (diffX = this.w - this.minW));
          else if (!this.dragState && this.maxW && this.w - diffX > this.maxW && this.l >= 0) this.offsetX = (diffX - (diffX = this.w - this.maxW));
          else if (this.fitParent && this.l + diffX < 0) this.offsetX = (diffX - (diffX = -this.l));
          this.calcMap & CALC_MASK.l && (this.l += diffX);
          this.calcMap & CALC_MASK.w && (this.w -= this.dragState ? 0 : diffX);
        }
        if (this.resizeState & ELEMENT_MASK['resizable-t'].bit) {
          if (!this.dragState && this.h - diffY < this.minH) this.offsetY = (diffY - (diffY = this.h - this.minH));
          else if (!this.dragState && this.maxH && this.h - diffY > this.maxH && this.t >= 0) this.offsetY = (diffY - (diffY = this.h - this.maxH));
          else if (this.fitParent && this.t + diffY < 0) this.offsetY = (diffY - (diffY = -this.t));
